 ...
 */
interface ExternalStockOverview {
  success: boolean
...
/**
 * Stock overview data from external API
 */
...
/**
 * Bounded cache for stock overview data (max 100 entries, 5 min TTL)
 */
const stockCache = new BoundedCache<ExternalStockOverview>(100, 5 * 60 * 1000)
...
/**
 * Fetch stock overview from external API
 *
 * @param symbol Stock symbol
 * @returns Stock overview data
 * @throws StockApiError if fetch fails
 */
async function fetchStockOverview(symbol: string): Promise<ExternalStockOverview> {
  const url = `${EXTERNAL_API_CONFIG.baseUrl}/th/stocks/${symbol}/overview`
  ...
  try {
    ...
    const data = (await response.json()) as ExternalStockOverview
...
/**
 * Fetch stock overview with retry logic
 *
 * @param symbol Stock symbol
 * @returns Stock overview data
 */
async function fetchWithRetry(symbol: string): Promise<ExternalStockOverview> {
  let lastError: Error | null = null

  for (let attempt = 0; attempt <= EXTERNAL_API_CONFIG.maxRetries; attempt++) {
    try {
      return await fetchStockOverview(symbol)
    } catch (error) {
...
/**
 * OPTIONS /api/stocks/[symbol]/overview
 *
