/**
 * Stock Yearly Data API Route
 *
 * GET /api/stocks/[symbol]/data/yearly
 * Proxies requests to external stock API and caches responses.
 *
 * External API: https://my-fon-stock-api.vercel.app/api/th/stocks/{symbol}/data/yearly
 */
...
import type { YearlyDataResponse } from '@/types/stock-price-api'
...
// Bounded cache for yearly data (100 entries, 30 min TTL)
const yearlyCache = new BoundedCache<YearlyDataResponse>(100, 30 * 60 * 1000)
...
function logError(symbol: string, message: string, error: unknown): void {
  ...
  if (process.env.NODE_ENV === 'production') {
    console.error(`[StockAPI] YearlyData ${symbol}: ${message}`, errorMsg)
  } else {
    console.error(`[StockAPI] YearlyData ${symbol}: ${message}`, error)
  }
}
...
async function fetchYearlyData(symbol: string): Promise<YearlyDataResponse> {
  const url = `${EXTERNAL_API_CONFIG.baseUrl}/th/stocks/${symbol}/data/yearly`
  ...
  try {
    ...
    const data = (await response.json()) as YearlyDataResponse
...
async function fetchWithRetry(symbol: string): Promise<YearlyDataResponse> {
  let lastError: Error | null = null

  for (let attempt = 0; attempt <= EXTERNAL_API_CONFIG.maxRetries; attempt++) {
    try {
      return await fetchYearlyData(symbol)
    } catch (error) {
...
export async function GET(
  ...
  try {
    ...
    if (!bypassCache) {
      const cached = yearlyCache.get(uppercaseSymbol)
      if (cached) {
        const response = cachedJson(
          {
            success: true,
    ...
    // Cache the result
    yearlyCache.set(uppercaseSymbol, externalData)
